 ...
 */
 ...
 class PackMan {
	...
	static public function loadPackFile($file) {
		if(is_readable(PackMan::PACK_PATH.$file.PackMan::JSON_EXT)) {
			$packfile = PackMan::PACK_PATH.$file.PackMan::JSON_EXT;
		}
		elseif(is_readable(PackMan::PACK_PATH.$file.PackMan::GZIP_EXT)) {
			$packfile = PackMan::PACK_PATH.$file.PackMan::GZIP_EXT;
		}
		else {
			throw new PackManException('Package "'. $file .'" not found in directory "'. __DIR__ .'/'. PackMan::PACK_PATH .'"');
		}
	...
	static private function _listPackPropertyTypes($entities) {
		...
		foreach($entities as $type) {
			$list = array_reduce($type->properties, 
