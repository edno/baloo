...
use Baloo\PackMan\PackManException as PackManException;

use Baloo\DataSourceManager as DataSourceManager;
use Baloo\BalooContext as BalooContext;
use Baloo\DataEntityType as DataEntityType;
 
class PackMan {
	
	const NAME = 'BALOO PACKage MANager';
	const VERSION = '0.20140501';
	...
	static public function loadPackFile($strfile) {
		try {
			if(is_readable(PackMan::PATH.$strfile.PackMan::JSON_EXT)) {
				$packfile = PackMan::PATH.$strfile.PackMan::JSON_EXT;
			}
			elseif(is_readable(PackMan::PATH.$strfile.PackMan::GZIP_EXT)) {
				$packfile = PackMan::PATH.$strfile.PackMan::GZIP_EXT;
			}
			else {
				throw new PackManException('Package "'. $strfile .'" not found in directory "'. PackMan::PATH .'"');
			}
	...
	static private function _createDataSourceFromPack($datasource) {
		...
		foreach($datasource->entities as $type) {
			...
			foreach($type->properties as $property) {
				DataSourceManager::insertDataTypeField($typeID, $property->name, $property->type, (isset($item->format)?$item->format:null), (isset($item->custom)?$item->custom:0));
			}
			$result = BalooContext::$pdo->commit();
		}
	...
	static private function _listPackPropertyTypes($entities) {
		...
		foreach($entities as $type) {
			$list = array_reduce(
					$type->properties, 
					function($result, $item) { array_push($result, array($item->type, (isset($item->format)?$item->format:null))); return $result; },
					$list);
		}
